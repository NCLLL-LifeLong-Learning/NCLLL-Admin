            ...
            <Table class="w-full bg-white">
                <TableHeader>
                    <TableRow class="w-full">
                        ...
                        <TableHead class="w-[10%]">{{ $t('category') }}</TableHead>
                ...
                <TableBody>
                    <TableRow v-for="(news, index) in newsList" :key="index">
                        ...
                        <TableCell class="">
                            {{ news.en.description ? news.en.description.slice(0, 120) + '...' : '' }}
                        </TableCell>
                        <TableCell>
                            {{ news?.category }}
                        </TableCell>
...
const onLoadNews = async () => {
    try {
        ...
        if (statusCode == 200) {
            ...
            newsList.value = data.results.filter((p: BlogModel) => p.category !== 'banner');
        }
    } catch (error) {
        console.error(error);
    } finally {
